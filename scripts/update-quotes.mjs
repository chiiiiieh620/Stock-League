...
function yahooSymbol(h){const s=String(h.symbol||'').toUpperCase();if(h.market==='TWSE')return `${s}.TW`;if(h.market==='TPEX')return `${s}.TWO`;return s;}
async function quote(ysym){
  ...
  const j=await r.json();const result=j?.chart?.result?.[0];let price=Number(result?.meta?.regularMarketPrice);
  if(!Number.isFinite(price)){const closes=result?.indicators?.quote?.[0]?.close||[];price=Number([...closes].reverse().find(v=>v!=null));}
  if(!Number.isFinite(price)||price<0)throw new Error('No price');return price;
}

const holdings=await sb('holdings?select=id,symbol,market');
const groups=new Map();
for(const h of holdings||[]){const ys=yahooSymbol(h);if(!groups.has(ys))groups.set(ys,[]);groups.get(ys).push(h);}
let updated=0;
for(const [ys,rows] of groups){
  try{
    const price=await quote(ys);
    for(const h of rows){
      await sb(`holdings?id=eq.${encodeURIComponent(h.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({current_price:price})});
      updated++;
    }
    console.log(`${ys}: ${price} -> ${rows.length} holding(s)`);
  }catch(e){console.warn(`${ys}: ${e.message}`);}
}
...
async function capture(type,key){const r=await fetch(`${SUPABASE_URL}/rest/v1/rpc/capture_champion`,{method:'POST',headers,body:JSON.stringify({p_period_type:type,p_period_key:key})});if(!r.ok)throw new Error(`snapshot ${type}: ${r.status} ${await r.text()}`);console.log(`${type} ${key}: ${await r.text()}`);}
